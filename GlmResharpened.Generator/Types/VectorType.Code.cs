...
internal partial class VectorType
{
	...
	public override IEnumerable<Member> GenerateMembers()
	{
		...
		if (BaseType.HasArithmetics && !BaseType.IsInteger && !BaseType.IsComplex)
			yield return new Function(BuiltinType.TypeBool, "ApproxEqual")
			{
				...
				CodeString = $"({lengthType.NameThat})Distance(lhs, rhs) <= eps",
		...
		yield return new ComponentWiseOperator(Fields, boolVType, "==", this, "lhs", this, "rhs", BaseType.EqualFormat);
		yield return new ComponentWiseOperator(Fields, boolVType, "!=", this, "lhs", this, "rhs", BaseType.NotEqualFormat);
		...
		if (BaseType.HasArithmetics)
		{
			...
			yield return new ComponentWiseStaticFunction(Fields, this, "HermiteInterpolationOrder3", this, "v", $"({intCastToByte}3 - {intCastToByte}2 * {{0}}) * {{0}} * {{0}}");
			yield return new ComponentWiseStaticFunction(Fields, this, "HermiteInterpolationOrder5", this, "v", $"(({intCastToByte}6 * {{0}} - {intCastToByte}15) * {{0}} + {intCastToByte}10) * {{0}} * {{0}} * {{0}}");
			...
			else
			{
				...
				yield return new ComponentWiseStaticFunction(Fields, integerVType, "Sign", this, "v", $"Math.Sign({intCastToDouble}{{0}})");
				...
				yield return new ComponentWiseStaticFunction(Fields, this, "Mix", this, "min", this, "max", this, "a", $"{{0}} * ({intCastToByte}1-{{2}}) + {{1}} * {{2}}");
				yield return new ComponentWiseStaticFunction(Fields, this, "Lerp", this, "min", this, "max", this, "a", $"{{0}} * ({intCastToByte}1-{{2}}) + {{1}} * {{2}}");
			...
			for (var comps = 2; comps <= 4; ++comps)
			{
				...
				{
					Static = true,
					Parameters = new[] { otherVec.Name + " c", Name + " r" },
				...
				if (comps != Components)
					...
					{
						Static = true,
						Parameters = new[] { Name + " c", otherVec.Name + " r" },
		...
		if (BaseType.IsFloatingPoint)
		{
			...
			yield return new ComponentWiseStaticFunction(Fields, this, "Floor", this, "v", $"({BaseTypeName})Math.Floor((double){{0}})");
			yield return new ComponentWiseStaticFunction(Fields, this, "Ceiling", this, "v", $"({BaseTypeName})Math.Ceiling((double){{0}})");
			yield return new ComponentWiseStaticFunction(Fields, this, "Round", this, "v", $"({BaseTypeName})Math.Round((double){{0}})");
			...
			yield return new ComponentWiseStaticFunction(Fields, this, "Fract", this, "v", $"({{0}} - ({BaseTypeName})Math.Floor((double){{0}}))");
			yield return new ComponentWiseStaticFunction(Fields, this, "Trunc", this, "v", $"({BaseTypeName})(long)({{0}})");
		}
		...
		if (BaseType.HasArithmetics)
		{
			...
			yield return new Property("Length", BaseType)
			{
				GetterLine = "(" + BaseTypeName + ")" + SqrtOf(intCastToDouble + Fields.Select(SqrOf).Aggregated(" + ")),
				Comment = "Returns the euclidean length of this vector."
			};

			yield return new Property("LengthSqr", BaseType)
			{
				GetterLine = "(" + BaseTypeName + ")" + Fields.Select(SqrOf).Aggregated(" + "),
				Comment = "Returns the squared euclidean length of this vector."
			};

			yield return new AggregatedProperty(Fields, "Sum", BaseType, "+", "Returns the sum of all components.");

			yield return new Property("Norm", BaseType)
			{
				GetterLine = "(" + BaseType.Name + ")" + SqrtOf(intCastToDouble + Fields.Select(SqrOf).Aggregated(" + ")),
				Comment = "Returns the euclidean norm of this vector."
			};
			yield return new Property("Norm1", BaseType)
			{
				GetterLine = Fields.Select(AbsString).Aggregated(" + "),
				Comment = "Returns the one-norm of this vector."
			};
			yield return new Property("Norm2", BaseType)
			{
				GetterLine = "(" + BaseType.Name + ")" + SqrtOf(intCastToDouble + Fields.Select(SqrOf).Aggregated(" + ")),
				Comment = "Returns the two-norm (euclidean length) of this vector."
			};
			yield return new Property("NormMax", BaseType)
			{
		...
		if (BaseType.HasArithmetics)
		{
			...
			yield return new Function(BaseType, "DistanceSqr")
			{
				Static = true,
				Parameters = this.LhsRhs(),
			...
			if (BaseType.IsSigned)
			{
				...
				yield return new Function(this, "Reflect")
				{
					...
					CodeString = $"I - {intCastToByte}2 * Dot(N, I) * N",
				...
				if (!BaseType.IsComplex)
				{
					...
					yield return new Function(this, "Refract")
					{
						...
						Code = new[]
							{
								"var dNI = Dot(N, I);",
							$"var k = {intCastToByte}1 - eta * eta * ({intCastToByte}1 - dNI * dNI);",
							$"if (k < {intCastToByte}0) return Zero;",
								string.Format("return eta * I - (eta * dNI + ({1}){0}) * N;", BaseType.IsComplex ? "Complex.Sqrt(k)" : SqrtOf($"{intCastToDouble}k"), BaseTypeName)
							},
					...
					yield return new Function(this, "FaceForward")
					{
						...
						CodeString = $"Dot(Nref, I) < {intCastToByte}0 ? N : -N",
