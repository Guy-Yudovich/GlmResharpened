...
internal partial class VectorType
{
		...
		{
			{
				...
				foreach (var field in Fields)
					yield return string.Format("Assert.That({1}, Is.EqualTo(v.{0}));", field, vals[0]).Indent();
				yield return "}";
			}

			{
				...
				foreach (var field in Fields)
					yield return string.Format("Assert.That({1}, Is.EqualTo(v.{0}));", field, it.MoveNext() ? it.Current : ZeroValue).Indent();
				yield return "}";
			}
	...
	private IEnumerable<string> TestIndexer
	{
		get
		{
			...
			for (var i = 0; i < vals.Length; ++i)
				yield return $"Assert.That({vals[i]}, Is.EqualTo(v[{i}]));";
			...
			foreach (var val in BaseType.ValuesSmallVals)
			{
				...
				yield return $"Assert.That({val}, Is.EqualTo(v[{i}]));";
			}
		}
	}
	...
	private IEnumerable<string> TestPropertyValue
	{
		get
		{
			...
			for (var i = 0; i < vals.Length; ++i)
				yield return $"Assert.That({vals[i]}, Is.EqualTo(vals[{i}]));";
	...
	private IEnumerable<string> TestStaticProperties
	{
		get
		{
			foreach (var f in Fields)
				yield return $"Assert.That({ZeroValue}, Is.EqualTo({NameThat}.Zero.{f}));";

			if (!string.IsNullOrEmpty(OneValue))
			{
				yield return "";
				foreach (var f in Fields)
					yield return $"Assert.That({OneValue}, Is.EqualTo({NameThat}.Ones.{f}));";

				foreach (var uf in Fields)
				{
					yield return "";
					foreach (var f in Fields)
						yield return $"Assert.That({(f == uf ? OneValue : ZeroValue)}, Is.EqualTo({NameThat}.Unit{uf.ToUpper()}.{f}));";
				}
			}
			...
			foreach (var constant in BaseType.TypeConstants)
			{
				yield return "";
				foreach (var f in Fields)
					yield return string.Format("Assert.That({0}.{2}, Is.EqualTo({1}.{2}.{3}));", BaseTypeName, NameThat, constant, f);
			}
		}
	}
	...
	private IEnumerable<string> TestStringInterop
	{
		get
		{
			...
			if (!BaseType.IsComplex && !BaseType.Generic)
			{
				...
				yield return string.Format("Assert.That(v, Is.EqualTo(v0));");
				yield return string.Format("Assert.That(v, Is.EqualTo(v1));");
				yield return "";
			...
			if (!BaseType.Generic)
			{
				...
				if (BaseType.HasFormatString && !BaseType.IsComplex)
				{
					...
					yield return string.Format("Assert.That(v, Is.EqualTo(v3));");
					yield return string.Format("Assert.That(v, Is.EqualTo(v4));");
					yield return "";
	...
	private IEnumerable<string> SerializationJson
	{
		get
		{
			...
			yield return "";
			yield return string.Format("Assert.That(v0, Is.EqualTo(v1));");

			yield return string.Format("Assert.That(s0, Is.EqualTo(s1));");
		}
	}
	...
	public override IEnumerable<TestFunc> GenerateTests()
	{
		...
		var intCastToSByte = BaseType.HasCastFromInt ? "" : "(sbyte)";
		...
		if (BaseType.HasArithmetics)
		{
			...
			yield return new InvariantTestFunc(this, "InvariantDouble", $"$V0 + $V0 == {intCastToByte}2 * $V0");
			yield return new InvariantTestFunc(this, "InvariantTriple", $"$V0 + $V0 + $V0 == {intCastToByte}3 * $V0");
		...
		else if (BaseType.IsFloatingPoint)
		{
			for (var i = 0; i < 5; ++i)
			{
				...
				var max = min + Random.Next(1, 5);
				var range = max - min;
				yield return new DistributionTestFunc(this, "RandomUniform" + i, "Random" + (i % 2 == 0 ? "" : "Uniform"), $"{intCastToSByte}({min}), {intCastToSByte}({max})", (min + max) / 2.0, range * range / 12.0);
			}
