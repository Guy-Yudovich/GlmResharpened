            ...
            get
            {
                foreach (var swizzle in Swizzle(0))
                {
                    ...
                    for (var i = 0; i < swizzle.Length; ++i)
                        if ($"var ov = {Construct(VectorType, vals)};".ToLower().Contains("null"))
                        {
                    ...
                    for (var i = 0; i < swizzle.Length; ++i)
                        yield return $"Assert.That({vals["xyzw".IndexOf(swizzle[i])]}, Is.EqualTo(v.{"xyzw"[i]}));".Indent();
                    yield return "}";
                }
            }
        }
        ...
        private IEnumerable<string> InlineXyzwTests
        {
            get
            {
                foreach (var swizzleBits in InlineSwizzle())
                {
                    ...
                    yield return "";
                    yield return $"Assert.That(v1, Is.EqualTo(v3));".Indent();
                    yield return "";
                    for (var i = 0; i < Components; ++i)
                        yield return $"Assert.That({(swizzleBits[i] == '1' ? subvals[swizzleBits.Substring(0, i).Count(c => c == '1')] : vals[i])}, Is.EqualTo(v0.{"xyzw"[i]}));".Indent();
                    yield return "";
                    for (var i = 0; i < swizzle.Length; ++i)
                        yield return $"Assert.That({vals["xyzw".IndexOf(swizzle[i])]}, Is.EqualTo(v2.{"xyzw"[i]}));".Indent();
                    yield return "}";
                }
            }
        }
        ...
        private IEnumerable<string> InlineRgbaTests
        {
            get
            {
                foreach (var swizzleBits in InlineSwizzle())
                {
                    ...
                    yield return "";
                    if (swizzle.Length == 1)
                        yield return $"Assert.That({vals["xyzw".IndexOf(swizzle[0])]}, Is.EqualTo(v2));".Indent();
                    else
                        for (var i = 0; i < swizzle.Length; ++i)
                            yield return $"Assert.That({vals["xyzw".IndexOf(swizzle[i])]}, Is.EqualTo(v2.{"xyzw"[i]}));".Indent();
                    yield return "}";
