...
internal class MatrixType : AbstractType
{
	...
	protected override IEnumerable<string> Body
	{
		get
		{
			var intCastToByte = BaseType.HasCastFromInt ? "" : "(byte)";
			var intCastToDouble = BaseType.HasCastFromInt ? "" : "(double)";

			// Indexer
			foreach (var line in $"Returns the number of Fields ({Columns} x {Rows} = {FieldCount}).".AsComment())
				yield return line;
			yield return $"public int Count => {FieldCount};";

			foreach (var line in "Gets/Sets a specific indexed component (a bit slower than direct access).".AsComment())
				yield return line;
			...
			foreach (var line in "Gets/Sets a specific 2D-indexed component (a bit slower than direct access).".AsComment())
				yield return line;
			...
			foreach (var line in "Returns true iff this equals rhs component-wise.".AsComment())
				yield return line;
			yield return $"public bool Equals({NameThat} rhs) => {Fields.Select(c => Comparer(c.ToString())).Aggregated(" && ")};";

			foreach (var line in "Returns true iff this equals rhs type- and component-wise.".AsComment())
				yield return line;
			...
			foreach (var line in "Returns true iff this does not equal rhs (component-wise).".AsComment())
				yield return line;
			yield return string.Format("public static bool operator !=({0} lhs, {0} rhs) => !lhs.Equals(rhs);", NameThat);

			foreach (var line in "Returns a hash code for this instance.".AsComment())
				yield return line;
			...
			if (BaseType.IsBool)
			{
				foreach (var line in "Returns the minimal component of this matrix.".AsComment())
					yield return line;
				yield return $"public {BaseTypeName} MinElement => {Fields.Aggregated(" && ")};";

				foreach (var line in "Returns the maximal component of this matrix.".AsComment())
					yield return line;
				yield return $"public {BaseTypeName} MaxElement => {Fields.Aggregated(" || ")};";

				foreach (var line in "Returns true if all component are true.".AsComment())
					yield return line;
				yield return $"public {BaseTypeName} All => {Fields.Aggregated(" && ")};";

				foreach (var line in "Returns true if any component is true.".AsComment())
					yield return line;
				yield return $"public {BaseTypeName} Any => {Fields.Aggregated(" || ")};";

				foreach (var line in "Executes a component-wise &amp;&amp;. (sorry for different overload but &amp;&amp; cannot be overloaded directly)".AsComment())
					yield return line;
			...
			if (BaseType.HasArithmetics)
			{
				...
				foreach (var line in "Returns the euclidean length of this matrix.".AsComment())
					yield return line;
				yield return string.Format("public {0} Length => ({0}){1};", BaseTypeName, SqrtOf(intCastToDouble + Fields.Select(SqrOf).Aggregated(" + ")));

				foreach (var line in "Returns the squared euclidean length of this matrix.".AsComment())
					yield return line;
				yield return $"public {BaseTypeName} LengthSqr => {Fields.Select(SqrOf).Aggregated(" + ")};";

				foreach (var line in "Returns the sum of all fields.".AsComment())
					yield return line;
				yield return $"public {BaseTypeName} Sum => {Fields.Aggregated(" + ")};";

				foreach (var line in "Returns the euclidean norm of this matrix.".AsComment())
					yield return line;
				yield return string.Format("public {0} Norm => ({0}){1};", BaseTypeName, SqrtOf(intCastToDouble + Fields.Select(SqrOf).Aggregated(" + ")));

				foreach (var line in "Returns the one-norm of this matrix.".AsComment())
					yield return line;
				yield return $"public {BaseTypeName} Norm1 => {Fields.Select(AbsString).Aggregated(" + ")};";

				foreach (var line in "Returns the two-norm of this matrix.".AsComment())
					yield return line;
				yield return string.Format("public {0} Norm2 => ({0}){1};", BaseTypeName, SqrtOf(intCastToDouble + Fields.Select(SqrOf).Aggregated(" + ")));

				foreach (var line in "Returns the max-norm of this matrix.".AsComment())
					yield return line;
				yield return $"public {BaseTypeName} NormMax => {NestedBiFuncFor(MathClass + ".Max({0}, {1})", FieldCount - 1, c => AbsString(FieldFor(c)))};";

				foreach (var line in "Returns the p-norm of this matrix.".AsComment())
					yield return line;
				...
				if (Rows == Columns)
				{
					foreach (var line in "Returns determinant of this matrix.".AsComment())
						yield return line;
					yield return $"public {BaseTypeName} Determinant => {HelperDet(HelperFieldsOf(Rows))};";

					if (BaseType.IsSigned)
					{
						foreach (var line in "Returns the adjunct of this matrix.".AsComment())
							yield return line;
						...
						foreach (var line in "Returns the inverse of this matrix (use with caution).".AsComment())
							yield return line;
				...
				for (var rcols = 2; rcols <= 4; ++rcols)
				{
					...
					foreach (var line in $"Executes a matrix-matrix-multiplication {NameThat} * {rhsType} -> {resultType}.".AsComment())
						yield return line;
				...
				foreach (var line in "Executes a matrix-vector-multiplication.".AsComment())
					yield return line;
				...
				{
					foreach (var line in "Executes a matrix-matrix-divison A / B == A * B^-1 (use with caution).".AsComment())
						yield return line;
					yield return string.Format("public static {0} operator/({0} A, {0} B) => A * B.Inverse;", NameThat);
				}

				// component-wise ops
				foreach (var line in "Executes a component-wise * (multiply).".AsComment())
					yield return line;
				...
				foreach (var line in "Executes a component-wise / (divide).".AsComment())
					yield return line;
				...
				foreach (var line in "Executes a component-wise + (add).".AsComment())
					yield return line;
				...
				foreach (var line in "Executes a component-wise - (subtract).".AsComment())
					yield return line;
                    ...
                    })
				{
					...
					var onlyScalar = op is "/" or "*";

					if (!onlyScalar)
					{
						foreach (var line in $"Executes a component-wise {opComment}.".AsComment())
							yield return line;
						yield return ComponentWiseOperator(op);
					}

					foreach (var line in $"Executes a component-wise {opComment} with a scalar.".AsComment())
						yield return line;
				...
				if (BaseType.IsInteger)
				{
					...
					foreach (var line in "Executes a component-wise left-shift with a scalar.".AsComment())
						yield return line;
					yield return ComponentWiseOperatorScalar("<<", "int");

					foreach (var line in "Executes a component-wise right-shift with a scalar.".AsComment())
						yield return line;
					yield return ComponentWiseOperatorScalar(">>", "int");
				}
				...
				if (!BaseType.IsComplex)
				{
						...
						})
					{
						...
						foreach (var line in $"Executes a component-wise {opComment} comparison.".AsComment())
							yield return line;
						yield return ComparisonOperator(op);
						foreach (var line in $"Executes a component-wise {opComment} comparison with a scalar.".AsComment())
							yield return line;
				...
				{
					// frustum
					foreach (var line in "Creates a frustrum projection matrix.".AsComment())
						yield return line;
					...
					yield return $"    m.m00 = ({intCastToByte}2 * nearVal) / (right - left);";
					yield return $"    m.m11 = ({intCastToByte}2 * nearVal) / (top - bottom);";
					...
					yield return $"    m.m32 = -({intCastToByte}2 * farVal * nearVal) / (farVal - nearVal);";
					yield return "    return m;";
					yield return "}";

					// infinite perspective
					foreach (var line in "Creates a matrix for a symmetric perspective-view frustum with far plane at infinite.".AsComment())
						yield return line;
					...
					// look at
					foreach (var line in "Build a look at view matrix.".AsComment())
						yield return line;
					...
					// ortho
					foreach (var line in "Creates a matrix for an orthographic parallel viewing volume.".AsComment())
						yield return line;
					yield return string.Format("public static {0} Ortho({1} left, {1} right, {1} bottom, {1} top, {1} zNear, {1} zFar)", NameThat, BaseTypeName);
					yield return "{";
					yield return "    var m = Identity;";
					yield return $"    m.m00 = {intCastToByte}2/(right - left);";
					yield return $"    m.m11 = {intCastToByte}2/(top - bottom);";
					...
					foreach (var line in "Creates a matrix for projecting two-dimensional coordinates onto the screen.".AsComment())
						yield return line;
					...
					// perspective
					foreach (var line in "Creates a perspective transformation matrix.".AsComment())
						yield return line;
					...
					yield return $"    m.m32 = ({BaseTypeName})( -({intCastToByte}2 * zFar * zNear) / (zFar - zNear) );";
					yield return "    return m;";
					yield return "}";

					// perspectiveFov
					foreach (var line in "Builds a perspective projection matrix based on a field of view.".AsComment())
						yield return line;
					yield return string.Format("public static {0} PerspectiveFov({1} fov, {1} width, {1} height, {1} zNear, {1} zFar)", NameThat, BaseTypeName);
					yield return "{";
					yield return $"    if (width <= {intCastToByte}0) throw new ArgumentOutOfRangeException(\"width\");";
					yield return $"    if (height <= {intCastToByte}0) throw new ArgumentOutOfRangeException(\"height\");";
					yield return $"    if (fov <= {intCastToByte}0) throw new ArgumentOutOfRangeException(\"fov\");";
					...
					yield return "    m.m23 = - 1;";
					yield return $"    m.m32 = - ({intCastToByte}2*zFar*zNear)/(zFar - zNear);";
					yield return "    return m;";
					yield return "}";
					...
					// project
					foreach (var line in "Map the specified object coordinates (obj.x, obj.y, obj.z) into window coordinates.".AsComment())
						yield return line;
					...
					yield return "{";
					yield return $"    var tmp = proj * (model * new {BaseType.Prefix}vec4(obj, {intCastToByte}1));";
					...
					// unproject
					foreach (var line in "Map the specified window coordinates (win.x, win.y, win.z) into object coordinates.".AsComment())
						yield return line;
					...
					yield return "{";
					yield return $"    var tmp = new {BaseType.Prefix + "vec4"}(win, {intCastToByte}1);";
					yield return "    tmp.x = (tmp.x - viewport.x) / viewport.z;";
					yield return "    tmp.y = (tmp.y - viewport.y) / viewport.w;";
					yield return $"    tmp = tmp * {intCastToByte}2 - {intCastToByte}1;";
					yield return "";
					...
					foreach (var line in "Map the specified window coordinates (win.x, win.y, win.z) into object coordinates (faster but less precise).".AsComment())
						yield return line;
					...
					// rotate
					foreach (var line in "Builds a rotation 4 * 4 matrix created from an axis vector and an angle in radians.".AsComment())
						yield return line;
					...
					yield return $"    var temp = ({intCastToByte}1 - c) * axis;";
					yield return "";
					yield return "    var m = Identity;";
					yield return "    m.m00 = c + temp.x * axis.x;";
					yield return $"    m.m01 = {intCastToByte}0 + temp.x * axis.y + s * axis.z;";
					yield return $"    m.m02 = {intCastToByte}0 + temp.x * axis.z - s * axis.y;";
					yield return "";
					yield return $"    m.m10 = {intCastToByte}0 + temp.y * axis.x - s * axis.z;";
					yield return "    m.m11 = c + temp.y * axis.y;";
					yield return $"    m.m12 = {intCastToByte}0 + temp.y * axis.z + s * axis.x;";
					yield return "";
					yield return $"    m.m20 = {intCastToByte}0 + temp.z * axis.x + s * axis.y;";
					yield return $"    m.m21 = {intCastToByte}0 + temp.z * axis.y - s * axis.x;";
					...
					{
						foreach (var line in $"Builds a rotation matrix around Unit{"XYZ"[axis]} and an angle in radians.".AsComment())
							yield return line;
					...
					// scale
					foreach (var line in "Builds a scale matrix by components x, y, z.".AsComment())
						yield return line;
					...
					foreach (var line in "Builds a scale matrix by vector v.".AsComment())
						yield return line;
					yield return $"public static {NameThat} Scale({BaseType.Prefix + "vec3"} v) => Scale(v.x, v.y, v.z);";
					foreach (var line in "Builds a scale matrix by uniform scaling s.".AsComment())
						yield return line;
					yield return $"public static {NameThat} Scale({BaseTypeName} s) => Scale(s, s, s);";

					// translation
					foreach (var line in "Builds a translation matrix by components x, y, z.".AsComment())
						yield return line;
					...
					foreach (var line in "Builds a translation matrix by vector v.".AsComment())
						yield return line;
