...
internal class QuaternionType : AbstractType
{
	...
	public string CompString => "xyzw"[..Components];
	...
	public override IEnumerable<Member> GenerateMembers()
	{
		...
		var intCastToFloat = BaseType.HasCastFromInt ? "" : "(float)";
		...
		if (BaseType.IsFloatingPoint)
		{
			...
			yield return new Constructor(this, Fields)
			{
				...
				Code = new[]
				{
						$"var c = {vec3Type.NameThat}.Cos(eulerAngle / {intCastToByte}2);",
						$"var s = {vec3Type.NameThat}.Sin(eulerAngle / {intCastToByte}2);",
					},
				Initializers = new[]
				{
		...
		if (BaseType.HasArithmetics)
		{
			...
			yield return new Operator(vec3Type, "*")
			{
				...
				Code = new[]
				{
					...
					$"return v + ((uv * q.w) + uuv) * {intCastToByte}2;"
				},
		...
		if (BaseType.IsFloatingPoint)
		{
			...
			yield return new Property("Axis", vec3Type)
			{
				Getter = new[]
				{
						$"var s1 = {intCastToByte}1 - w * w;",
						$"if (s1 < {intCastToByte}0) return {vec3Type.Name}.UnitZ;",
						$"var s2 = {intCastToByte}1 / ({BaseTypeName}){SqrtOf($"{intCastToDouble}s1")};",
		...
		if (BaseType.IsFloatingPoint)
		{
			yield return new Property("ToMat3", mat3Type)
			{
				GetterLine = Construct(mat3Type,
					$"{intCastToByte}1 - {intCastToByte}2 * (y*y + z*z)",
					$"{intCastToByte}2 * (x*y + w*z)",
					$"{intCastToByte}2 * (x*z - w*y)",

					$"{intCastToByte}2 * (x*y - w*z)",
					$"{intCastToByte}1 - {intCastToByte}2 * (x*x + z*z)",
					$"{intCastToByte}2 * (y*z + w*x)",

					$"{intCastToByte}2 * (x*z + w*y)",
					$"{intCastToByte}2 * (y*z - w*x)",
					$"{intCastToByte}1 - {intCastToByte}2 * (x*x + y*y)"
					),
		...
		if (BaseType.HasArithmetics)
		{
			...
			if (BaseType.IsFloatingPoint)
			{
				...
				yield return new Function(this, "Squad")
				{
					...
					CodeString = $"Mix(Mix(q1, q2, h), Mix(s1, s2, h), {intCastToByte}2 * ({intCastToByte}1 - h) * h)",
