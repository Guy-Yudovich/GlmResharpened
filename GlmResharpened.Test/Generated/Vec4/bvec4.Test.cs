#pragma warning disable
using System;
using System.Collections;
using System.Collections.Generic;
using System.Globalization;
using System.Runtime.InteropServices;
using System.Runtime.Serialization;
using System.Numerics;
using System.Linq;
using NUnit.Framework;
using Newtonsoft.Json;
using GlmResharpened;
using GlmResharpened.Swizzle;

namespace GlmResharpenedTest.Generated.Vec4;

[TestFixture]
public class BoolVec4Test
{

    [Test]
    public void Constructors()
    {
        {
            var v = new bvec4(true);
            Assert.That(true, Is.EqualTo(v.x));
            Assert.That(true, Is.EqualTo(v.y));
            Assert.That(true, Is.EqualTo(v.z));
            Assert.That(true, Is.EqualTo(v.w));
        }
        {
            var v = new bvec4(true, false, true, false);
            Assert.That(true, Is.EqualTo(v.x));
            Assert.That(false, Is.EqualTo(v.y));
            Assert.That(true, Is.EqualTo(v.z));
            Assert.That(false, Is.EqualTo(v.w));
        }
        {
            var v = new bvec4(new bvec2(false, false));
            Assert.That(false, Is.EqualTo(v.x));
            Assert.That(false, Is.EqualTo(v.y));
            Assert.That(false, Is.EqualTo(v.z));
            Assert.That(false, Is.EqualTo(v.w));
        }
        {
            var v = new bvec4(new bvec3(false, false, true));
            Assert.That(false, Is.EqualTo(v.x));
            Assert.That(false, Is.EqualTo(v.y));
            Assert.That(true, Is.EqualTo(v.z));
            Assert.That(false, Is.EqualTo(v.w));
        }
        {
            var v = new bvec4(new bvec4(true, true, false, false));
            Assert.That(true, Is.EqualTo(v.x));
            Assert.That(true, Is.EqualTo(v.y));
            Assert.That(false, Is.EqualTo(v.z));
            Assert.That(false, Is.EqualTo(v.w));
        }
    }

    [Test]
    public void Indexer()
    {
        var v = new bvec4(false, true, false, false);
        Assert.That(false, Is.EqualTo(v[0]));
        Assert.That(true, Is.EqualTo(v[1]));
        Assert.That(false, Is.EqualTo(v[2]));
        Assert.That(false, Is.EqualTo(v[3]));
        
        Assert.Throws<ArgumentOutOfRangeException>(() => { var s = v[-2147483648]; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { v[-2147483648] = false; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { var s = v[-1]; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { v[-1] = false; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { var s = v[4]; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { v[4] = false; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { var s = v[2147483647]; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { v[2147483647] = false; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { var s = v[5]; } );
        Assert.Throws<ArgumentOutOfRangeException>(() => { v[5] = false; } );
        
        v[1] = false;
        Assert.That(false, Is.EqualTo(v[1]));
        v[1] = true;
        Assert.That(true, Is.EqualTo(v[1]));
    }

    [Test]
    public void PropertyValues()
    {
        var v = new bvec4(true, true, true, true);
        var vals = v.Values;
        Assert.That(true, Is.EqualTo(vals[0]));
        Assert.That(true, Is.EqualTo(vals[1]));
        Assert.That(true, Is.EqualTo(vals[2]));
        Assert.That(true, Is.EqualTo(vals[3]));
        Assert.That(vals.SequenceEqual(v.ToArray()));
    }

    [Test]
    public void StaticProperties()
    {
        Assert.That(false, Is.EqualTo(bvec4.Zero.x));
        Assert.That(false, Is.EqualTo(bvec4.Zero.y));
        Assert.That(false, Is.EqualTo(bvec4.Zero.z));
        Assert.That(false, Is.EqualTo(bvec4.Zero.w));
        
        Assert.That(true, Is.EqualTo(bvec4.Ones.x));
        Assert.That(true, Is.EqualTo(bvec4.Ones.y));
        Assert.That(true, Is.EqualTo(bvec4.Ones.z));
        Assert.That(true, Is.EqualTo(bvec4.Ones.w));
        
        Assert.That(true, Is.EqualTo(bvec4.UnitX.x));
        Assert.That(false, Is.EqualTo(bvec4.UnitX.y));
        Assert.That(false, Is.EqualTo(bvec4.UnitX.z));
        Assert.That(false, Is.EqualTo(bvec4.UnitX.w));
        
        Assert.That(false, Is.EqualTo(bvec4.UnitY.x));
        Assert.That(true, Is.EqualTo(bvec4.UnitY.y));
        Assert.That(false, Is.EqualTo(bvec4.UnitY.z));
        Assert.That(false, Is.EqualTo(bvec4.UnitY.w));
        
        Assert.That(false, Is.EqualTo(bvec4.UnitZ.x));
        Assert.That(false, Is.EqualTo(bvec4.UnitZ.y));
        Assert.That(true, Is.EqualTo(bvec4.UnitZ.z));
        Assert.That(false, Is.EqualTo(bvec4.UnitZ.w));
        
        Assert.That(false, Is.EqualTo(bvec4.UnitW.x));
        Assert.That(false, Is.EqualTo(bvec4.UnitW.y));
        Assert.That(false, Is.EqualTo(bvec4.UnitW.z));
        Assert.That(true, Is.EqualTo(bvec4.UnitW.w));
    }

    [Test]
    public void Operators()
    {
        var v1 = new bvec4(false, false, true, false);
        var v2 = new bvec4(false, false, true, false);
        var v3 = new bvec4(false, true, false, false);
        Assert.That((v1 == new bvec4(v1)).AllTrue);
        Assert.That((v2 == new bvec4(v2)).AllTrue);
        Assert.That((v3 == new bvec4(v3)).AllTrue);
        Assert.That((v1 == v2).AllTrue);
        Assert.That((v1 != v3).AllTrue);
        Assert.That((v2 != v3).AllTrue);
    }

    [Test]
    public void StringInterop()
    {
        var v = new bvec4(true, true, true, true);
        
        var s0 = v.ToString();
        var s1 = v.ToString("#");
        
        var v0 = bvec4.Parse(s0);
        var v1 = bvec4.Parse(s1, "#");
        Assert.That(v, Is.EqualTo(v0));
        Assert.That(v, Is.EqualTo(v1));
        
        var b0 = bvec4.TryParse(s0, out v0);
        var b1 = bvec4.TryParse(s1, "#", out v1);
        Assert.That(b0);
        Assert.That(b1);
        Assert.That(v, Is.EqualTo(v0));
        Assert.That(v, Is.EqualTo(v1));
        
        b0 = bvec4.TryParse(null, out v0);
        Assert.That(!b0);
        b0 = bvec4.TryParse("", out v0);
        Assert.That(!b0);
        b0 = bvec4.TryParse(s0 + ", 0", out v0);
        Assert.That(!b0);
        
        Assert.Throws<NullReferenceException>(() => { bvec4.Parse(null); });
        Assert.Throws<FormatException>(() => { bvec4.Parse(""); });
        Assert.Throws<FormatException>(() => { bvec4.Parse(s0 + ", 0"); });
        
        var s2 = v.ToString(";", CultureInfo.InvariantCulture);
        Assert.That(s2.Length > 0);
    }

    [Test]
    public void SerializationJson()
    {
        var v0 = new bvec4(false, true, false, false);
        var s0 = JsonConvert.SerializeObject(v0);
        
        var v1 = JsonConvert.DeserializeObject<bvec4>(s0);
        var s1 = JsonConvert.SerializeObject(v1);
        
        Assert.That(v0, Is.EqualTo(v1));
        Assert.That(s0, Is.EqualTo(s1));
    }

}
